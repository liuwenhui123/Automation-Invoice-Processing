...
- 2026-05-20：商家/开票日期解析问题优先检查 `invoice_app/parser.py`。已覆盖带空格中文日期、以及“个人 + 销售方公司名 + 单个销售方税号”的特殊版式；测试入口为 `tests/test_parser.py`。
- 2026-05-20：发布节奏默认保守。不是每次代码改动都打新 Release；除非用户明确要求、需要新的 exe 下载包，或改动适合作为一个新的对外版本节点，否则只做提交和推送。
